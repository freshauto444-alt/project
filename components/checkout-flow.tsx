...
export default function CheckoutFlow({ car, onClose }: CheckoutFlowProps) {
  ...
  const isValidPhone = (phone: string) => /^\+?[\d\s\-()]{7,}$/.test(phone.trim())

  const canNext =
    (step === 0 && form.firstName.trim() && form.lastName.trim()
     && isValidEmail(form.email) && isValidPhone(form.phone)) ||
      ...
      <section className="py-12 px-6">
        <div className="mx-auto max-w-2xl">
          ...
          >
            ...
            <div className="flex items-center gap-4 rounded-xl border border-border bg-card p-4 w-full max-w-sm">
              ...
              <div className="text-right">
                <div className="text-sm font-semibold text-primary">{formatPrice(calcTotalCost(car.price).total)}</div>
                <div className="text-[9px] text-muted-foreground/70">під ключ</div>
              </div>
            </div>
            <button
              onClick={onClose}
      ...
      <div className="mx-auto max-w-2xl">
            ...
            {step === 0 && (
              <motion.div key="contact" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.2 }} className="space-y-4">
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  <div>
                    <label htmlFor="co-firstName" className="mb-1 block text-[10px] text-muted-foreground uppercase tracking-wider">{"Ім'я"}</label>
                    <input id="co-firstName" name="firstName" type="text" required autoComplete="given-name" value={form.firstName} onChange={(e) => setForm({ ...form, firstName: e.target.value })} className="w-full rounded-xl border border-border bg-transparent px-4 py-2.5 text-xs text-foreground placeholder:text-muted-foreground/30 focus:border-primary/20 focus:outline-none transition-colors" placeholder="Олександр" />
                  </div>
                  <div>
                    <label htmlFor="co-lastName" className="mb-1 block text-[10px] text-muted-foreground uppercase tracking-wider">Прізвище</label>
                    <input id="co-lastName" name="lastName" type="text" required autoComplete="family-name" value={form.lastName} onChange={(e) => setForm({ ...form, lastName: e.target.value })} className="w-full rounded-xl border border-border bg-transparent px-4 py-2.5 text-xs text-foreground placeholder:text-muted-foreground/30 focus:border-primary/20 focus:outline-none transition-colors" placeholder="Коваленко" />
                  </div>
                </div>
                <div>
                  <label htmlFor="co-email" className="mb-1 block text-[10px] text-muted-foreground uppercase tracking-wider">Email</label>
                  <input id="co-email" name="email" type="email" required autoComplete="email" inputMode="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} className="w-full rounded-xl border border-border bg-transparent px-4 py-2.5 text-xs text-foreground placeholder:text-muted-foreground/30 focus:border-primary/20 focus:outline-none transition-colors" placeholder="oleksandr@example.ua" />
                </div>
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  <div>
                    <label htmlFor="co-phone" className="mb-1 block text-[10px] text-muted-foreground uppercase tracking-wider">Телефон</label>
                    <input id="co-phone" name="phone" type="tel" required autoComplete="tel" inputMode="tel" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} className="w-full rounded-xl border border-border bg-transparent px-4 py-2.5 text-xs text-foreground placeholder:text-muted-foreground/30 focus:border-primary/20 focus:outline-none transition-colors" placeholder="+380 67 XXX XXXX" />
                  </div>
                  <div>
                    <label htmlFor="co-company" className="mb-1 block text-[10px] text-muted-foreground uppercase tracking-wider">Компанія</label>
                    <input id="co-company" name="company" type="text" autoComplete="organization" value={form.company} onChange={(e) => setForm({ ...form, company: e.target.value })} className="w-full rounded-xl border border-border bg-transparent px-4 py-2.5 text-xs text-foreground placeholder:text-muted-foreground/30 focus:border-primary/20 focus:outline-none transition-colors" placeholder="Необов'язково" />
                  </div>
                </div>
              </motion.div>
            )}
                  ...
                  {paymentMethods.map((m) => (
                    <button
                      ...
                      className={`flex w-full items-center gap-4 rounded-xl border p-4 text-left transition-all duration-200 cursor-pointer ${
                        form.paymentMethod === m.id ? "border-primary/20 bg-primary/[0.04]" : "border-border hover:border-border"
                      }`}
                    >
            ...
            {step === 2 && (
              <motion.div key="contract" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.2 }}>
                ...
                <div className="rounded-xl bg-white/[0.02] p-5 mb-5 text-[11px] text-muted-foreground space-y-2 max-h-44 overflow-y-auto font-mono">
                  ...
                  <p>VIN: {car.vin}</p>
                  <p>Вартість під ключ: {formatPrice(calcTotalCost(car.price).total)} ({formatPrice(car.price)} з ЄС)</p>
                ...
                <button
                  ...
                  className={`flex w-full items-center gap-4 rounded-xl border p-4 transition-all duration-200 cursor-pointer ${
                    form.signed ? "border-primary/20 bg-primary/[0.04]" : "border-border hover:border-border"
                  }`}
                >
        ...
        <div className="mt-5 flex items-center justify-between">
          <button
            ...
            className="flex items-center gap-2 rounded-full border border-border px-5 py-2.5 text-xs font-medium text-muted-foreground hover:text-foreground transition-colors cursor-pointer"
          >
                ...
                try {
                  ...
                  const res = await fetch("/api/orders", {
                    ...
                    body: JSON.stringify({
                      ...
                      total_price: car?.price ? calcTotalCost(car.price).total : null,
                  ...
                  toast.success("Замовлення оформлено! Менеджер зв'яжеться з вами.")
                  setDone(true)
                } catch (e: any) {
                  const msg = e?.name === "AbortError" ? "Таймаут з'єднання. Спробуйте ще раз." : (e?.message || "Помилка при оформленні")
                  toast.error(msg)
                } finally {
                  setSubmitting(false)
                }
