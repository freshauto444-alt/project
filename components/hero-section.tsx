...
export default function HeroSection({ onExplore, onFinder }: HeroSectionProps) {
  ...
  /* Load iframe IMMEDIATELY on mount — hero video is the first thing users see.
     Delay was adding ~300ms perceived lag. YouTube iframe itself is non-blocking
     (it renders in its own browsing context). */
  useEffect(() => {
    setLoadIframe(true)
  }, [])
  ...
  /* Toggle navbar white text when hero is visible.
     CRITICAL: cleanup must remove the class — otherwise when user navigates away
     from home (client-side nav keeps the navbar mounted), the class lingers and
     all navbar text stays white, making it invisible on light theme. */
  useEffect(() => {
    ...
    return () => {
      obs.disconnect()
      // Always remove class on unmount — hero is leaving the page.
      if (navbar) navbar.classList.remove("navbar-hero-active")
    }
  }, [iframeReady])
      ...
      {loadIframe && (
        ...
        >
          <iframe
            ...
            loading="eager"
            onLoad={() => {
              /* Show video as soon as onLoad fires — no artificial delay */
