...
B. РІВНО 3 моделі. За замовчуванням — мінімум 2 різні марки. ВИНЯТОК: у сценарії РОЗДВОЄННЯ (див. СТРАТЕГІЮ) дві з трьох карток можуть бути ОДНІЄЇ марки+моделі (різні покоління) — це навмисно, не помилка.
...
• РОЗДВОЄННЯ — якщо клієнт назвав КОНКРЕТНУ марку+модель і його бюджет КОНФЛІКТУЄ з бажаною свіжістю/роком (бажане НОВЕ покоління не вписується в бюджет, АЛЕ СТАРІШЕ покоління тієї Ж моделі — вписується) → дай ДВІ картки ТІЄЇ Ж марки+моделі:
   – Картка A «у бюджет»: СТАРІШЕ покоління, що реально вписується. yearRange = ВЛАСНІ роки того покоління (саме freshness-обмеження клієнта ти тут і послаблюєш — НЕ перетинай з year_from клієнта). budgetFit:"fits", feasibilityWarning:null. whyRecommended коротко: це покоління вкладається у ваш бюджет.
   – Картка B «свіже»: бажане НОВЕ покоління з ЧЕСНОЮ вищою ціною. budgetFit:"over" + overBy + feasibilityWarning (на скільки дорожче і чому свіже не влазить у бюджет).
   – 3-тя картка = найкращий РЕАЛЬНИЙ варіант У БЮДЖЕТ; ПЕРЕВАГА — ТА Ж марка (інша модель/версія, що вписується, напр. сусідня M-модель); cross-make лише якщо сильної однопородної у бюджет немає.
• Якщо клієнт назвав КОНКРЕТНУ марку+модель, але в бюджет НЕ вписується ЖОДНЕ її покоління (навіть найстаріше) → перша картка = та модель з чесним budgetFit:"over" + overBy, інші 2 = реальні альтернативи в бюджеті ("fits"). (Роздвоєння тут не має сенсу — у бюджет цієї моделі не існує взагалі.)
...
2. yearRange = перетин меж покоління з обмеженнями клієнта:
   ...
   • ВИНЯТОК (картка A у сценарії РОЗДВОЄННЯ): НЕ перетинай з year_from клієнта — бери ВЛАСНІ роки старішого покоління. Саме цей freshness-фільтр ти послаблюєш, щоб влізти в бюджет; інакше вийде порожній/інвертований діапазон і картку буде відкинуто.
