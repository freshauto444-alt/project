...
/* ───── Hyper Fullscreen Gallery ─────
 *  Native-feeling photo viewer: swipe/drag, pinch-zoom, pan-when-zoomed,
 *  keyboard nav, adjacent-image preload, progress dots, thumbnail strip.
 */
function FullscreenGallery({ images, startIndex, onClose }: {
  ...
  const [zoom, setZoom] = useState(1) // 1 = fit, 2.5 = zoomed
  const [pan, setPan] = useState({ x: 0, y: 0 })
  const [dragOffset, setDragOffset] = useState(0) // horizontal drag for swipe feedback
  const stripRef = useRef<HTMLDivElement>(null)
  const touchState = useRef<{ x: number; y: number; t: number; pinchDist: number | null; baseZoom: number; basePan: { x: number; y: number } }>({
    x: 0, y: 0, t: 0, pinchDist: null, baseZoom: 1, basePan: { x: 0, y: 0 },
  })
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === " ") { e.preventDefault(); setZoom(z => (z > 1 ? 1 : 2.5)); setPan({ x: 0, y: 0 }) }
      if (e.key === "Home") { setIdx(0); setZoom(1); setPan({ x: 0, y: 0 }) }
      if (e.key === "End") { setIdx(images.length - 1); setZoom(1); setPan({ x: 0, y: 0 }) }
    }
    window.addEventListener("keydown", handler)
    return () => window.removeEventListener("keydown", handler)
  }, [images.length, onClose, go])
  ...
  const onTouchMove = (e: React.TouchEvent) => {
    if (e.touches.length === 2 && touchState.current.pinchDist != null) {
      ...
      setZoom(Math.max(1, Math.min(4, touchState.current.baseZoom * ratio)))
    } else if (e.touches.length === 1) {
      const dx = e.touches[0].clientX - touchState.current.x
      const dy = e.touches[0].clientY - touchState.current.y
      if (zoom > 1) {
        // Pan when zoomed
        setPan({ x: touchState.current.basePan.x + dx, y: touchState.current.basePan.y + dy })
      } else {
        // Swipe feedback (rubber-band)
        setDragOffset(dx)
      }
    }
  }
  const onTouchEnd = () => {
    if (zoom <= 1) {
      // Commit swipe if past threshold
      const dx = dragOffset
      const fast = Date.now() - touchState.current.t < 300
      if (dx < -60 || (fast && dx < -25)) go(1)
      else if (dx > 60 || (fast && dx > 25)) go(-1)
      setDragOffset(0)
    }
  ...
  const [mouseDrag, setMouseDrag] = useState<{ x: number; y: number; basePan: { x: number; y: number } } | null>(null)
  const onMouseDown = (e: React.MouseEvent) => {
    if (zoom > 1) { setMouseDrag({ x: e.clientX, y: e.clientY, basePan: pan }); e.preventDefault() }
  }
  const onMouseMove = (e: React.MouseEvent) => {
    if (mouseDrag) setPan({ x: mouseDrag.basePan.x + e.clientX - mouseDrag.x, y: mouseDrag.basePan.y + e.clientY - mouseDrag.y })
  }
  const onMouseUp = () => setMouseDrag(null)
  ...
  return (
    ...
    >
      {/* Preload adjacent frames — instant swipe transitions */}
      {prev && <link rel="preload" as="image" href={prev} />}
      {next && <link rel="preload" as="image" href={next} />}
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-black/80 backdrop-blur-sm border-b border-white/[0.06]">
        ...
        <div className="flex items-center gap-1.5">
          <button
            onClick={() => { setZoom(z => (z > 1 ? 1 : 2.5)); setPan({ x: 0, y: 0 }) }}
            className="rounded-lg p-2 text-white/50 hover:text-white hover:bg-white/[0.06] transition-colors cursor-pointer"
      ...
      {/* Main image */}
      <div
        className="flex-1 relative flex items-center justify-center overflow-hidden touch-none select-none"
      ...
      >
        <AnimatePresence mode="wait">
          <motion.img
            ...
            style={{
              transform: `translate3d(${dragOffset + pan.x}px, ${pan.y}px, 0) scale(${zoom})`,
              transformOrigin: "center center",
              transition: dragOffset === 0 ? "transform 0.18s ease-out" : "none",
            }}
            className={`max-h-full max-w-full object-contain ${zoom > 1 ? "cursor-grab active:cursor-grabbing" : "cursor-zoom-in"}`}
            onClick={() => { if (zoom === 1) { setZoom(2.5) } else { setZoom(1); setPan({ x: 0, y: 0 }) } }}
        ...
        {/* Progress dots (desktop only, hidden on small screens where thumb strip takes over) */}
        {images.length > 1 && images.length <= 12 && (
          <div className="pointer-events-none absolute bottom-4 left-1/2 hidden -translate-x-1/2 items-center gap-1.5 sm:flex">
            {images.map((_, i) => (
              <span
                key={i}
                className={`h-1.5 rounded-full transition-all ${
                  i === idx ? "w-6 bg-white/90" : "w-1.5 bg-white/30"
                }`}
              />
            ))}
          </div>
      ...
      <div ref={stripRef} className="flex items-center gap-1.5 px-4 py-3 bg-black/80 backdrop-blur-sm overflow-x-auto scrollbar-thin border-t border-white/[0.06]">
        {images.map((img, i) => (
          ...
          >
            <img src={img} alt={`Thumb ${i + 1}`} crossOrigin="anonymous" className="h-full w-full object-cover" draggable={false} loading="lazy" />
          </button>
        ))}
      </div>
    </motion.div>
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          {car.image && (
            <img
              ...
              onError={e => {
                const el = e.currentTarget
                if (el.src !== car.image) el.src = car.image!
              }}
            />
          )}
        </div>
...
function TradeInSection({ car }: { car: CarType }) {
  ...
  // Trade-in subtracts from TURNKEY (what user actually pays), not raw EU.
  const turnkeyPrice = car.price ? calcTotalCost(car.price).total : 0
  const finalPrice = Math.max(0, turnkeyPrice - tradeValue)
    ...
    <div className="flex flex-col gap-5">
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          <label className="text-xs text-muted-foreground">{t("modal.yourCarValue", language)}</label>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="rounded-xl bg-secondary/50 p-4 text-center">
          <div className="text-[10px] text-muted-foreground mb-1">{t("modal.yourCar", language)}</div>
        ...
        <div className="rounded-xl bg-primary/[0.06] border border-primary/10 p-4 text-center">
          <div className="text-[10px] text-muted-foreground mb-1">{t("modal.toPay", language)}</div>
      ...
      <div className="rounded-xl bg-secondary/30 p-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <div className="text-xs font-medium text-foreground mb-0.5">{t("modal.tradeInHowWorks", language)}</div>
            <p className="text-[11px] text-muted-foreground leading-relaxed">{t("modal.tradeInHelp", language)}</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        <a href="tel:+380987081919" className="flex items-center gap-1.5 rounded-xl bg-primary px-4 py-2.5 text-xs font-medium text-primary-foreground hover:brightness-110 transition-all">
          <Phone className="h-3 w-3" />{t("modal.requestReport", language)}
        </a>
        <a href="https://www.instagram.com/freshauto_ua/" target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 rounded-xl border border-white/[0.08] px-4 py-2.5 text-xs font-medium text-foreground hover:bg-white/[0.04] transition-all">
          <MessageCircle className="h-3 w-3" />{t("modal.writeTo", language)}
        </a>
      </div>
    </div>
  )
                  ...
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                      <div className="flex items-baseline gap-2">
                        <div className="text-2xl font-semibold text-primary tracking-tight tabular-nums">{formatPrice(calcTotalCost(car.price).total)}</div>
                        <div className="text-xs font-medium text-muted-foreground/80">під ключ</div>
                      </div>
                      <div className="text-[11px] text-muted-foreground/60 mt-0.5 tabular-nums">
                        {formatPrice(car.price)} з ЄС · розрахунок нижче
                      </div>
                    </div>
